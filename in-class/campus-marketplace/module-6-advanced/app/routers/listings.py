...
from app.rate_limit import create_listing_rate_limit
...
@router.post(
    ...
    status_code=201,
    dependencies=[Depends(create_listing_rate_limit)],   # path-operation dependency: return value unused
)
async def create_listing(
    ...
    # Live-notify anyone watching the /ws/listings feed. Fire-and-forget from the
    # request's point of view: the HTTP response doesn't wait on broadcast delivery.
    await manager.broadcast("listings", {"event": "new_listing", "id": listing.id, "title": listing.title})
    return listing
